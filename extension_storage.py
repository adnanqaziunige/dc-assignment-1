...
class Node:
    ...
    max_concurrent_transfers: int  # Maximum simultaneous uploads/downloads for a node
    ...
    def __post_init__(self):
        ...
        self.active_uploads = set() # Tracks block IDs being uploaded
        self.active_downloads = set()  # Tracks block IDs being downloaded
    ...
    def find_block_to_back_up(self):
        ...
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
            if held_locally and peer is None:  # We have the block locally, but it is not backed up remotely
                return block_id
        return None

    def schedule_next_upload(self, sim: Backup,active_uploads):
        ...
        while done and len(self.active_uploads) < self.max_concurrent_transfers:
            # print("while upload")
            # print("while transfer", self.max_concurrent_transfers)
        ...
        # first find if we have a backup that a remote node needs
            for peer, block_id in self.remote_blocks_held.items():
                ...
                if peer.online and not peer.local_blocks[block_id] and (block_id not in  peer.active_downloads and block_id not in  active_uploads) and len(peer.active_downloads) < peer.max_concurrent_transfers:
                    self.active_uploads.add(block_id)  # Peer tracks as a download
                    peer.active_downloads.add(block_id)  # Peer tracks as a download
                    # if(len(self.active_uploads)>0):
                    #     print("len2")
                    # print("schedule_next_upload")
                    print(f"{self.name} to {peer.name}: {block_id}")
                    sim.schedule_transfer(self, peer, block_id, restore=True)
                    done=True # we have found our upload, we stop
                    break
            # if done:
            #     continue
            ...
            for peer in sim.nodes:
                ...
                if (peer is not self and peer.online and peer not in remote_owners and (block_id not in  peer.active_downloads and block_id not in  active_uploads) 
                        and peer.free_space >= self.block_size and len(peer.active_downloads) < peer.max_concurrent_transfers):
                    self.active_uploads.add(block_id)  # Peer tracks as a download
                    peer.active_downloads.add(block_id)  # Peer tracks as a download
                    # print("schedule_next_upload")
                    print(f"{self.name} to {peer.name}: {block_id}")
    ...
    def schedule_next_download(self, sim: Backup,active_downloads):
        ...
        while done and len(self.active_downloads) < self.max_concurrent_transfers:
            # print("while download")
            # print("while transfer", self.max_concurrent_transfers)
            done=False

            for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
                if not held_locally and peer is not None and peer.online and (block_id not in  peer.active_uploads and block_id not in  active_downloads)  and  len(peer.active_uploads) < peer.max_concurrent_transfers:
                    self.active_downloads.add(block_id)  # Add to active downloads
                    peer.active_uploads.add(block_id)  # Add to active downloads
                    print(f"{peer.name} to {self.name}: {block_id}")

                    sim.schedule_transfer(peer, self, block_id, restore=True)
                    done=True # we have found our upload, we stop
                    # print("schedule_next_download")
                    break
            # if done:
        ...
        # try to back up a block for a remote node
            for peer in sim.nodes:
                if   (peer is not self and peer.online and len(peer.active_uploads) < peer.max_concurrent_transfers
                    and self.free_space >= peer.block_size):
                    block_id = peer.find_block_to_back_up()
                    if block_id is not None and (block_id not in  peer.active_uploads and block_id not in  active_downloads) :
                        self.active_downloads.add(block_id)  # Add to active downloads
                        peer.active_uploads.add(block_id)  # Add to active downloads
                        # print("schedule_next_download")
                        print(f"{peer.name} to {self.name}: {block_id}")
...
class TransferComplete(Event):
    ...
    def process(self, sim: Backup):
        sim.log_info(f"{self.__class__.__name__} from {self.uploader} to {self.downloader}: {self.block_id}")
        if self.canceled:
        ...
        if not uploader.online or not downloader.online:
            sim.schedule(uploader.arrival_time,self)
        self.update_block_state()
        uploader.current_upload = downloader.current_download = None
        ucopy=uploader.active_downloads

        dcopy=downloader.active_downloads
        downloader.active_downloads.remove(self.block_id)
        uploader.active_uploads.remove(self.block_id)
        uploader.schedule_next_upload(sim,ucopy)
        downloader.schedule_next_download(sim,dcopy)
        ...
        sim.log_file(
            f"{self.__class__.__name__} | Uploader: {self.uploader} | Downloader: {self.downloader} | Block ID: {self.block_id} | Time: {sim.t:.2f}"
        )
...
class BlockRestoreComplete(TransferComplete):
    ...
    def update_block_state(self):
        ...
        if sum(owner.local_blocks) == owner.k:  # we have exactly k local blocks, we have all of them then
            print(f"{owner} has fully recovered its data.")
