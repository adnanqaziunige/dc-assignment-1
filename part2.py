...
class MonitorSIR(Event):
    ...
    def __init__(self,lambd ,interval=1):
        self.interval = interval
        self.lambd = lambd

    def process(self, sim):
        analyze_queue_lengths(sim,self.lambd)
...
def analyze_queue_lengths(sim,lambd):
    queue_lengths = [len(q) for q in sim.queues]
    queue_snapshots[lambd].append(queue_lengths)
...
for idx, d in enumerate(choices, 1):
    ...
    for lambd in lambdas:
        ...
        sim.schedule(0, MonitorSIR(lambd,plot_interval))
